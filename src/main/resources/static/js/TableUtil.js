...
let GRID = {  
    ...
    set : function(obj){
    	...
    	if(this.excel != "") {
    		...
    		document.getElementById(obj.gridSearch).querySelector(".gridSearch").after(button);
    ...
    getData : function(){
		...
		let p = {
			...
			, success : function (opt,result) {
    	    	_this.setTable(result.data);
    	    	if(result.data.length <= 0) return;
    	    	if(_this.isPaging && _this.init) _this.setPaging(result.data[0].totalCnt);
    	    }
		}
		AjaxUtil.post(p);
    },
    ...
    setPaging : function(totalCnt){
		console.log(this.params, totalCnt);
    	let _this = this;
    	let current = parseInt(_this.params.page)+1;
    	this.pageCnt = Math.ceil(totalCnt / 10);
