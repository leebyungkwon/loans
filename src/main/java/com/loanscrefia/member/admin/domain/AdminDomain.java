...
public class AdminDomain extends BaseDomain{
	...
	@Pattern(regexp = "^[a-zA-Z0-9가-힣]*${1,30}", message = "부서명을 1~30자리로 입력해 주세요. (특수문자 제외)")
	private String 	deptNm;					// 부서명
	
	@NotBlank(message = "담당자이름 을 입력하세요.")
	@Pattern(regexp = "^[a-zA-Z가-힣]*${1,20}", message = "담당자이름을 1~20자리로 입력해 주세요. (특수문자 제외)")
	private String 	memberName;			// 담당자이름
	
	@NotBlank(message = "직위명 을 입력해주세요.")
	@Pattern(regexp = "^[a-zA-Z가-힣]*${1,20}", message = "직위명을 1~20자리로 입력해 주세요. (특수문자 제외)")
